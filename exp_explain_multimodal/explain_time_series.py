...
from model import BertSelfOutput, AttentionBlock, BertPooler, PositionalEncoding, PhysiSelfAttention, LayerNorm
...
def flip(model, attribution, data, y_true, fracs, mask_token, flip_case, random_order=False, device='cpu'):
    ...
    inputs0 = torch.tensor(np.float32(data['physi']), requires_grad=True).unsqueeze(0).to(device)

    y0 = model(inputs0).squeeze().detach().cpu().numpy()

    if random_order == False:
        if flip_case == 'generate':
            # add tokens from big to small
            inds_sorted = np.argsort(attribution)[::-1]
        elif flip_case == 'pruning':
            # remove tokens from small to big
    ...
    for frac in fracs:
        ...
        if flip_case == 'pruning':
            ...
            for i in inds_flip:
                # time series mask token shape is (1,76)
                inputs[:, i] = mask_token
...
def run(config):
    ...
    mask_token = np.load("exp_explain_multimodal/data/physi_mask_token.npy")
    mask_token = torch.tensor(np.float32(mask_token)).to(device)

    for fold_i in range(5):
        with open(f'data/dataset/pickle/align_ts/fold_{fold_i}_ts.dat', 'rb') as f:
            data_dict = pickle.load(f)
        test_data = data_dict['test']

        print(f"Begin to test in {fold_i}")

        ts_params = torch.load(os.path.join(config['path_setting']['physi_ckpt_path'], f"best_model_state_{fold_i}.bin")).state_dict()

        physi_config = config['model_setting']['physi']

        # set model for explain
        physi_config["detach_layernorm"] = False  # Detaches the attention-block-output LayerNorm
        physi_config["detach_kq"] = False
        model = PhysiSelfAttention(physi_config)
        model.load_state_dict(ts_params)
        model.to(device)
        models = {'none': model}

        # Transformer Model
        physi_config["detach_layernorm"] = False  # Detaches the attention-block-output LayerNorm
        physi_config["detach_kq"] = True
        model = PhysiSelfAttention(physi_config)
        model.load_state_dict(ts_params)
        model.to(device)
        ...
        print('Detach SM +  LN')
        # Transformer Model
        physi_config["detach_layernorm"] = True  # Detaches the attention-block-output LayerNorm
        physi_config["detach_kq"] = True
        model = PhysiSelfAttention(physi_config)
        model.load_state_dict(ts_params)
        model.to(device)
        ...
        # Transformer Model
        physi_config["detach_layernorm"] = True  # Detaches the attention-block-output LayerNorm
        physi_config["detach_mean"] = False  # Detaches the attention-block-output LayerNorm
        physi_config["detach_kq"] = True
        model = PhysiSelfAttention(physi_config)
        model.load_state_dict(ts_params)
        model.to(device)
        ...
        for flip_case in ['pruning']:
            ...
            for case, random_order in [#('random', True),
                ...
                for i in range(total_test_num):
                    ...
                    input_tensor = torch.tensor(np.float32(x["physi"]), requires_grad=True).unsqueeze(0).to(device)

                    y_true = torch.tensor(x['mortality']).to(device)
                    if case == 'ig':
                        # inputs_baseline = torch.concat(input_tensor.shape[1] * [mask_token]).unsqueeze(dim=0)
                        #inputs_baseline = torch.from_numpy(np.random.random(size=(input_tensor.shape))).float().to(device)
                        inputs_baseline = torch.from_numpy(np.zeros(shape=(input_tensor.shape))).float().to(device)
            ...
            pickle.dump(all_flips, open(f'exp_explain_multimodal/ts_explain/fold_{fold_i}_all_flips_{flip_case}_ig.p', 'wb'))

if __name__ == "__main__":
    args = get_args()
    config = get_config_from_json(args.config)
    save_dir = 'exp_explain_multimodal/ts_explain'
