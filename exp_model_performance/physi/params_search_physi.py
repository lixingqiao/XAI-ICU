...
from utils import MultiModalReader, MultiModalDataset, PhysiDiscretizer, PhysiNormalizer

from model import PhysiSelfAttention, PhysiRNN, PhysiLSTM, PhysiGRU, PhysiRCNN, PhysiAttRNN
...
def train_epoch(model, data_loader, optimizer, device, epoch, num_epochs, n_examples, verbose=True):
    ...
    for itr, sample in enumerate(train_bar):
        ...
        input_data = sample["physi"].type(torch.float).to(device)
...
def load_data(train_df, test_df, n_fold, pickle_path, verbose=True):
    pickle_file = os.path.join(pickle_path, n_fold + "_ts" + ".dat")
    if os.path.exists(pickle_file):
        ...
        train_reader = MultiModalReader(dataset_dir="data/",
                                        ...
                                        period_length=24.0, modal=["physi"])
        ...
        discretizer_header = discretizer.transform(train_reader.read_example(0)["physi"])[1].split(',')
        ...
        # Load data
        train_dataset = MultiModalDataset.load(train_reader, physi_discretizer=discretizer, physi_normalizer=normalizer,
                                               modals=["physi"])
        test_datset = MultiModalDataset.load(test_reader, physi_discretizer=discretizer, physi_normalizer=normalizer,
                                             modals=["physi"])
...
class Objective:
    ...
    def __call__(self, trial):
        ...
        # ------------------------------------------------------------
        # training setting
        config['exp_setting']['lr'] = trial.suggest_loguniform('lr', 1e-5, 1e-2)
        ...
        if config['exp_setting']['model_name'] == 'physi_attention':
            ## model setting
            config['model_setting']['xai_transformer']['n_blocks'] = trial.suggest_int("n_blocks", 1, 6)
            ...
            config['model_setting']['RNN']['n_blocks'] = trial.suggest_int("n_blocks", 1, 6)
            ...
            config['model_setting']['LSTM']['n_blocks'] = trial.suggest_int("n_blocks", 1, 6)
            ...
            config['model_setting']['GRU']['n_blocks'] = trial.suggest_int("n_blocks", 1, 6)
            ...
            config['model_setting']['RCNN']['n_blocks'] = trial.suggest_int("n_blocks", 1, 6)
            ...
            config['model_setting']['AttRNN']['n_blocks'] = trial.suggest_int("n_blocks", 1, 6)
        ...
        for fold_i, n_fold in enumerate(kfold_df.columns):
            ...
            augmentation_training_size = 10 * len(samples_weight)
            ...
            # ------------------------------------------------------------
            if config['exp_setting']['model_name'] == 'physi_attention':
                model = PhysiSelfAttention(config['model_setting']['xai_transformer'])
            elif config['exp_setting']['model_name'] == 'physi_rnn':
                model = PhysiRNN(config['model_setting']['RNN'])
            elif config['exp_setting']['model_name'] == 'physi_lstm':
                model = PhysiLSTM(config['model_setting']['LSTM'])
            elif config['exp_setting']['model_name'] == 'physi_rcnn':
                model = PhysiRCNN(config['model_setting']['RCNN'])
            elif config['exp_setting']['model_name'] == 'physi_gru':
                model = PhysiGRU(config['model_setting']['GRU'])
            elif config['exp_setting']['model_name'] == 'physi_attrnn':
                model = PhysiAttRNN(config['model_setting']['AttRNN'])
            elif config['exp_setting']['model_name'] == 'physi_transformer':
                model = PhysiTransformer(config['model_setting']['Transformer'])
            else:
...
def optuna_search(config):
    ...
    study.optimize(Objective(config), n_trials=50)
