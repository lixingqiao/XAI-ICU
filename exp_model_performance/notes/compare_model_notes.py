...
from train_eval_notes import train_and_eval
...
def parse_config(model_name, trial_params, config):
    ...
    if model_name == 'notes_attention':
        ...
        config['model_setting']['xai_transformer']['all_head_size'] = trial_params['all_head_size']
...
def main(study_dir, config):
    for each_study in os.listdir(study_dir):
        model_name = "notes_" + each_study.split(".")[0].split("_")[-1]
        ...
        for random_seed in range(0, 5):
            ...
            config['path_setting']['ckpt_dir'] = os.path.join(f"exp_model_performance/ckpt/note/{model_name}", f"seed_{config['exp_setting']['random_seed']}_trial/")
...
if __name__ == "__main__":
    study_path = "exp_model_performance/study/note"
