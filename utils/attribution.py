...
def _compute_integrated_gradient(batch_x, batch_blank, model, idx):
    mean_grad = 0
    n = 10

    for i in range(1, n + 1):
        batch_tmp = batch_blank + i / n * (batch_x - batch_blank)
        x = batch_tmp.detach()
    ...
    attribution = integrated_gradients.sum(2).detach().cpu().numpy().squeeze()
    return attribution


def _compute_integrated_gradient_nlp(batch_x, batch_blank, model, idx):
    mean_grad = 0
    n = 50
    ...
    for i in range(1, n + 1):
        ...
        x = batch_tmp.detach()
        x.requires_grad = True
        y = model.compute_with_embedding_input(x)[:, idx]
        (grad,) = torch.autograd.grad(y, x)
        mean_grad += grad / n
